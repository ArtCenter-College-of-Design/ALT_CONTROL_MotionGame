...
public class KinectModelControllerV2 : MonoBehaviour {
	...
	private Vector3 m_localScale_orig;
	private Vector3 m_localScale_faceToface;

	// Use this for initialization
	void Start () {

		m_localScale_orig = transform.localScale;
	...
	void Update () {
			...
			)
		{
			...
			if (m_faceToFace)
			{//模型与真人面对面
				m_localScale_faceToface = m_localScale_orig;
				m_localScale_faceToface.z = -m_localScale_faceToface.z;
				transform.localScale = m_localScale_faceToface;
			}
			else
			{//模型与真人同向
				transform.localScale = m_localScale_orig;
	...
	private void swapLeftRight(ref GameObject left, ref GameObject right)
	{
		GameObject gb_tmp = left;
		left = right;
		right = gb_tmp;
	}
}
